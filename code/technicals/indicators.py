...
def BollingerBands(df: pd.DataFrame, n=20, s=2): #n=number for the rolling window, s for the deviation
    typical_p = (df.mid_c + df.mid_h + df.mid_l) / 3
    stddev = typical_p.rolling(window=n).std()
    df['BB_MA'] = typical_p.rolling(window=n).mean() #BB = BollingerBands
