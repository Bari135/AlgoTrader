...
class OandaApi:
    ...
    def make_request(self, url, verb = 'get', code=200,
        ...
        try:
            ...
            if verb == "put":
                response = self.session.put(full_url, params=params,
                                        data=data, headers=headers)
    ...
    ## summary, instruments יהיה רלוונטי לבקשות
    ...
    def fetch_candles(self, pairName, count=10, granularity="H1",
                      price = "MBA", date_f=None, date_t=None):
        url = f"instruments/{pairName}/candles"
        params = dict(
            granularity = granularity,
            price = price
        )

        if date_f is not None and date_t is not None:  #או שנשתמש בטווח תאריכים או בכמות נרות
    ...
    def place_trade(self, pair_name: str, units: float, direction: int,
        ...
        instrument = ic.instrument_dict[pair_name]
    ...
    def close_trade(self, trade_id):
        ...
        if ok:
            ...
            print(f"Failed to clode trade: {trade_id}")
