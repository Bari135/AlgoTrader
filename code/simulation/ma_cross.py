...
def load_price_data(pair, granularity, ma_list):
    ...
    for ma in ma_list:
        df[get_ma_col(ma)] = df.mid_c.rolling(window=ma).mean() #mean-ממוצע
    df.dropna(inplace=True) #rolling(window) - לקחת כל פעמים רק 20
...
def process_macro(results_list, filename): #(full filename)
...
def process_results(results_list, filepath):
    process_macro(results_list, get_fullname(filepath, "ma_res")) #מטעין את הנתוני סימולציה לקובץ
    process_trades(results_list, get_fullname(filepath, "ma_trades"))   #מטעין את הפעולות מסחר לקובץ
