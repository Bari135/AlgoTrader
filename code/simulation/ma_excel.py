...
def add_pair_charts(df_ma_res, df_ma_trades, writer):
    ...
    for _, row in df_temp.iterrows():   #_ is for the index, we dont need 
...
def add_pair_sheets(df_ma_res, writer):
    for p in df_ma_res.pair.unique(): #add sheet for each pair
        tdf = df_ma_res[df_ma_res.pair == p]
        tdf.to_excel(writer, sheet_name = p, index = False)

def prepare_data(df_ma_res, df_ma_trades, writer):
    df_ma_res.sort_values(by = ['pair', 'total_gain'],
                          ascending = [True, False],
                          inplace = True)
    df_ma_trades['time'] = [x.replace(tzinfo =None) for x in df_ma_trades['time']]

def procces_data(df_ma_res, df_ma_trades, writer):
    prepare_data(df_ma_res, df_ma_trades, writer)
...
def create_excel(df_ma_res, df_ma_trades, granularity):
    ...
    procces_data(df_ma_res[df_ma_res.granularity == granularity].copy(),
