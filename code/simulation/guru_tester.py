...
def apply_signals(df, PROFIT_FACTOR, sig): #sig is the function
...
def create_signals(df, time_d=1):
    ...
    df_signals.rename(columns={
        'bid_c' : 'start_price_BUY',    #I'm trying to buy so bid is the highest price people giving for that pair, i join them.
        'ask_c' : 'start_price_SELL',   #I'm trying to sell so ask is the lowest price people giving for that pair
        'm5_start' : 'time'
    }, inplace=True)
    return df_signals
...
class GuruTester:
    ...
    def prepare_data(self):
        ...
        apply_signals(self.df_big,          #adds the SIGNAL, SL, TP columns
                    self.PROFIT_FACTOR,
                    self.apply_signal)
