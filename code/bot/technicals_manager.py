...
def apply_signal(row, trade_settings: TradeSettings):
    ...
    if row.SPREAD <= trade_settings.maxspread and row.GAIN >= trade_settings.mingain:
        if row.mid_c > row.BB_UP and row.mid_o < row.BB_UP:
            return defs.SELL
        if row.mid_c < row.BB_LW and row.mid_o > row.BB_LW:
            return defs.BUY
    return defs.NONE
...
def fetch_candles(pair, row_count, candle_time, granularity,
    ...
    if df is None or df.shape[0] == 0:
        log_message("tech manager fetch_candles failed to get candles", pair)
        return None
    
    if df.iloc[-1].time != candle_time:
        log_message(f"tech manager fetch_candles {df.iloc[-1].time} not correct" , pair)
        return None
    
    return df

def get_trade_decision(candle_time, pair, granularity, api:OandaApi,
                       trade_setting:TradeSettings, log_message):
    max_rows = trade_setting.n_ma + ADDROWS
    log_message(f"tech_manager: max_rows:{max_rows} candle_time:{candle_time} granularity:{granularity} pair_name:{pair}", pair)

    df = fetch_candles(pair, max_rows, candle_time, granularity, api, log_message)

    if df is not None:
        last_row = process_candles(df,pair, trade_setting, log_message)
